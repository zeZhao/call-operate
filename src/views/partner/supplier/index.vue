<template>
  <div class="user">
    ...
    >
      ...
      <el-table-column prop="createTime" label="有效时间" width="150"/>
      <el-table-column prop="createTime" label="开户时间" width="150" />
      <el-table-column label="操作" width="130" fixed="right">
        <template slot-scope="scope">
          <el-button
            @click="_mxEdit(scope.row, 'supplyId')"
            type="text"
            size="small"
            >修改</el-button
          >
          ...
          <!-- <el-button
            @click="
              _mxDeleteItem('supplyId', scope.row.supplyId, false, true)
            "
            type="text"
            size="small"
            >删除
...
export default {
  ...
  data() {
    return {
      ...
      //接口地址
      searchAPI: {
        namespace: "corpSupply",
        list: "list",
        add: "updateAndSaveCorpSupply",
        edit: "updateAndSaveCorpSupply",
        detele: "delete",
      },
      // 列表参数
      namespace: "",
  ...
  methods: {
    ...
    rechargeSubmit(){
      ...
      let params = Object.assign(form,{corpId:this.row.corpId,userId:this.row.userId,supplyId:this.row.supplyId})
      // console.log(,'====')
      this.$http.corpSupply.cropSupplyRecharge(params).then(res=>{ 
        if(res.state === '200'){
